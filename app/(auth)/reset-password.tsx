...
import { CheckmarkCircle01Icon, LockPasswordIcon, ViewIcon, ViewOffIcon } from "@hugeicons/core-free-icons";
        ...
        {done ? (
          <View className="bg-white rounded-[32px] p-6 border border-neutral-100 gap-4">
            ...
            <View className="items-center py-4 gap-3">
              ...
              <Text className="text-foreground text-xl text-center" style={styles.bold}>Password updated!</Text>
            ...
            <Pressable className="bg-accent rounded-2xl py-4 items-center" onPress={() => router.replace("/(auth)/login" as any)}>
              <Text className="text-accent-foreground text-base" style={styles.bold}>Sign In</Text>
            </Pressable>
          </View>
        ) : (
          <View className="bg-white rounded-[32px] p-6 border border-neutral-100 gap-4">
            ...
            <View className="items-center gap-1">
              <View className="w-14 h-14 rounded-2xl bg-accent/15 items-center justify-center">
                <HugeiconsIcon icon={LockPasswordIcon} size={28} color="#ffc400" />
              </View>
              <Text className="text-foreground text-xl mt-2" style={styles.bold}>New Password</Text>
              <Text className="text-muted-foreground text-sm text-center" style={styles.body}>Enter your new password below.</Text>
            </View>

            <View>
              <Text className="text-xs text-muted-foreground mb-1.5" style={styles.semibold}>New Password</Text>
            ...
            <View>
              ...
              <View className="flex-row items-center bg-neutral-50 rounded-2xl px-4 border border-neutral-100">
                <HugeiconsIcon icon={LockPasswordIcon} size={18} color="#ffc400" />
                <TextInput className="flex-1 py-3.5 px-3 text-foreground" style={styles.body} placeholder="••••••••" placeholderTextColor="#A3A3A3" secureTextEntry={!showConfirm} value={confirmPassword} onChangeText={setConfirmPassword} />
                <Pressable onPress={() => setShowConfirm(!showConfirm)}>
                  <HugeiconsIcon icon={showConfirm ? ViewOffIcon : ViewIcon} size={18} color="#A3A3A3" />
                </Pressable>
              </View>
            </View>
            ...
            >
              {loading ? <ActivityIndicator color="#171717" /> : (
                <Text className="text-accent-foreground text-base" style={styles.bold}>Update Password</Text>
              )}
            </Pressable>
          </View>
        )}
