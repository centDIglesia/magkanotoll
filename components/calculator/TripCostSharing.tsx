...
import { UserGroupIcon, FuelStationIcon, MinusSignIcon, PlusSignIcon } from "@hugeicons/core-free-icons";
...
const GAS_PRICE_DEFAULT = 65;

export default function TripCostSharing({ totalToll, gasLiters }: Props) {
  const [passengers, setPassengers] = useState(2);
  const [gasPrice, setGasPrice] = useState(String(GAS_PRICE_DEFAULT));

  const gasCost = gasLiters ? gasLiters * (parseFloat(gasPrice) || GAS_PRICE_DEFAULT) : 0;
    ...
    <View className="bg-white rounded-3xl border border-neutral-100 p-5 mt-4">
      ...
      {gasLiters && (
        <View className="flex-row items-center bg-neutral-50 rounded-2xl px-4 border border-neutral-100 mb-3">
          ...
          <Text
            className="text-muted-foreground text-xs ml-2 mr-1"
            style={styles.body}
          >
            ₱
          </Text>
      ...
      <View className="flex-row items-center justify-between bg-neutral-50 rounded-2xl px-4 py-3 border border-neutral-100 mb-4">
        ...
        <View className="flex-row items-center gap-4">
          ...
          <Text
            className="text-foreground text-base w-6 text-center"
            style={styles.bold}
          >
            {passengers}
          </Text>
      ...
      <View className="gap-2 mb-4">
        <View className="flex-row justify-between">
          ...
          <Text className="text-foreground text-xs" style={styles.body}>
            ₱{totalToll.toFixed(2)}
          </Text>
        </View>
        {gasLiters && (
          <View className="flex-row justify-between">
            <Text className="text-muted-foreground text-xs" style={styles.body}>
              Gas ({gasLiters.toFixed(1)}L × ₱
              {parseFloat(gasPrice) || GAS_PRICE_DEFAULT})
            </Text>
            <Text className="text-foreground text-xs" style={styles.body}>
              ₱{gasCost.toFixed(2)}
            </Text>
        ...
        <View className="flex-row justify-between">
          ...
          <Text className="text-foreground text-xs" style={styles.bold}>
            ₱{totalCost.toFixed(2)}
          </Text>
        </View>
      </View>
